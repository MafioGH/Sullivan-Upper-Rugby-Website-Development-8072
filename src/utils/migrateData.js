...
const coaches=JSON.parse(localStorage.getItem('rugbyCoaches') || '[]')
const media=JSON.parse(localStorage.getItem('rugbyMedia') || '[]')

console.log(`📊 Found ${fixtures.length} fixtures,${results.length} results,${players.length} players,${coaches.length} coaches,${media.length} media items`)
...
await supabase.from('coaches_rugby12345').delete().neq('id',0)
...
const transformedCoaches=coaches.map(c=> ({
id: c.id,
name: c.name,
role: c.role,
...
photo: c.photo || '',
bio: c.bio || '',
headCoach: c.headCoach || false
}))

const {error: coachesError}=await supabase
.from('coaches_rugby12345')
.insert(transformedCoaches)

if (coachesError) {
console.error("❌ Coaches migration error:",coachesError)
throw coachesError
}
console.log("✅ Coaches migrated successfully")
}
