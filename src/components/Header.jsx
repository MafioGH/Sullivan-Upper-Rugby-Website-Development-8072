...
const {FiMenu,FiX,FiHome,FiCalendar,FiTrophy,FiUsers,FiSettings,FiShield,FiBarChart,FiUpload}=FiIcons;
...
{path: '/photo-manager',label: 'Media',icon: FiUpload},
...
>
<SafeIcon icon={item.path==='/admin' && isAdminAuthenticated ? FiShield : item.icon} className="w-4 h-4" />
...
>
<SafeIcon icon={item.path==='/admin' && isAdminAuthenticated ? FiShield : item.icon} className="w-5 h-5" />
<span>{item.label}</span>
{item.path==='/admin' && isAdminAuthenticated && (
